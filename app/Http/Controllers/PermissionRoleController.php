...
class PermissionRoleController extends Controller
{
    public function index(){ //listar todos los elementos almacenados en esa tabla
        return response()->json(PermissionRole::all(),200); //Permission es el modelo. 200: es el OK
    }

    public function show($id){
        $the_permission = PermissionRole::find($id);
    ...
    public function store(Request $request){
        $the_permission=PermissionRole::create($request->all());
